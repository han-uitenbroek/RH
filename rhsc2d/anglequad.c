/* ------- file: -------------------------- anglequad.c -------------
       ...
       Last modified: Tue Apr 21 14:38:54 2009 --

       --------------------------                      ----------RH-- */

/* --- Determine angular quadrature and fill ray stucture.
  ...
  See: B. G. Carlson in "Methods of Computational Physics", Vol 1,
       ...
       Weights are normalized to yield 1.0 when integrated over two
       octants, which for symmetry reasons represent all space.
       --                                              -------------- */
...
void getAngleQuadr(Geometry *geometry)
{
  ...
  case SET_GL: geometry->Nrays =
    2 * atmos.angleSet.Ninclination * atmos.angleSet.Nazimuth;
    break;
  case SET_A2: geometry->Nrays = 2*NRO_A2;  break;
  case SET_A4: geometry->Nrays = 2*NRO_A4;  break;
  case SET_A6: geometry->Nrays = 2*NRO_A6;  break;
  case SET_A8: geometry->Nrays = 2*NRO_A8;  break;
  case SET_B4: geometry->Nrays = 2*NRO_B4;  break;
  case SET_B6: geometry->Nrays = 2*NRO_B6;  break;
  case SET_B8: geometry->Nrays = 2*NRO_B8;  break;
  case NO_SET:
  ...
  case SET_VERTICAL:
    if (input.Eddington) {
      geometry->mux[0] = 1.0 / sqrt(3.0);
      geometry->muy[0] = 1.0 / sqrt(3.0);
  ...
  /* --- Normalize, remember we integrate over only two octants -- -- */
