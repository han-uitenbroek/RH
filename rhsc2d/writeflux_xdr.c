/* ------- file: -------------------------- writeflux_xdr.c ---------
       ...
       Last modified: Tue Nov  7 11:39:47 2000 --
...
bool_t writeFlux(char *fileName)
{
  ...
  bool_t result = TRUE;
  int    Nrays = geometry.Nrays, Nx = geometry.Nx;
  double *flux, *wmuz;
  FILE  *fp_flux;
  XDR    xdrs;
  ...
  for (nspect = 0;  nspect < spectrum.Nspect;  nspect++) {
    for (k = 0;  k < Nx;  k++) flux[k] = 0.0;
    for (mu = 0;  mu < geometry.Nrays;  mu++) {
      for (k = 0;  k < Nx;  k++)
	flux[k] += spectrum.I[nspect*Nrays + mu][k] * wmuz[mu];
    }
    for (k = 0;  k < Nx;  k++) flux[k] *= 2.0 * PI;
    result &= xdr_vector(&xdrs, (char *) flux, Nx,
